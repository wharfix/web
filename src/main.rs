...
struct WharfixTemplateResponse<T: Template>(T);
struct WharfixRedirectResponse(String);

impl<T: Template + 'static> WharfixTemplateResponse<BaseTemplate<T>> {
    fn wrap(content: T) -> Self where T: Template {
        Self(BaseTemplate { content })
    }
}

impl WharfixRedirectResponse {
...
//struct WharfixWebResult(Result<WharfixTemplateResponse, WharfixWebError>);
type WharfixTemplateResult<T> = Result<WharfixTemplateResponse<T>, WharfixWebError>;
type WharfixRedirectResult = Result<WharfixRedirectResponse, WharfixWebError>;

impl<T: Template> Responder for WharfixTemplateResponse<T> {
    type Error = Error;
    ...
    fn respond_to(self, _req: &HttpRequest) -> Self::Future {
        ready(Ok(
            HttpResponse::Ok()
                        ...
                        .body(self.0.render().unwrap())
        ))
    }
}

impl Responder for WharfixRedirectResponse {
    type Error = Error;
    type Future = Ready<Result<HttpResponse, Error>>;
...
async fn front_page(info: web::Query<FeedbackInfo>) -> WharfixTemplateResult<BaseTemplate<FrontPageTemplate>> {

    let msg = info.msg.as_ref().and_then(|m| WharfixWebError::from_str(m).ok()).and_then(|m| Some(format!("{}", m.to_string())));

    Ok(WharfixTemplateResponse::wrap(FrontPageTemplate {
        msg
    }))
}
...
async fn repo_submit(session: Session, params: web::Form<RepoParams>) -> WharfixRedirectResult {
    use mysql::TxOpts;
    use regex::Regex;
   ...
   // ***
    ...
    Ok(WharfixRedirectResponse::redirect("/manage"))
}
...
async fn manage_page(session: Session, info: web::Query<FeedbackInfo>) -> WharfixTemplateResult<BaseTemplate<ManagePageTemplate>> {
   ...
   Ok(WharfixTemplateResponse::wrap(content))
}
...
fn db_connect() -> Pool {
   let config = Config::get();
   Pool::new(mysql::Opts::from_url(&config.db_conn_string).unwrap()).unwrap()
}
